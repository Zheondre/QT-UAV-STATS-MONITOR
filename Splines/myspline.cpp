...
void mySpline::updateValues(){
    ...
    if(m_chart){
//if there arnt any added items dont start
        //Generate the data
        ...
        for (QSplineSeries *it : series){
            ...
            //QMetaObject::invokeMethod(m_chart,example[i], Qt::AutoConnection, Q_RETURN_ARG(QSplineSeries*, it), Q_ARG(int, i)); //no such method error
            QMetaObject::invokeMethod(m_chart,"series", Qt::AutoConnection, Q_RETURN_ARG(QSplineSeries*, it), Q_ARG(int, i)); //no such method error
            i++;
        }
        //series.at(0)->name()
        ...
        dateTimeAxis->setMax(QDateTime::fromMSecsSinceEpoch(m_data1.last().x()));//crashes
